...
class EcommerceController extends Controller
{
     ...
     public function products(Request $r){
      ...
      $products =Post::latest()->where('type',2)->where('status','<>','temp')
        ->where(function($q) use ($r,$allPer) {
            ...
            if($r->category_id){
                $q->whereHas('productCtgs',function($qq) use($r){
                  $qq->where('reff_id',$r->category_id);
                });
            }
            ...
            if($r->status){
                if($r->status=='newarrival'){
                    $q->where('new_arrival',true); 
                }elseif($r->status=='fetured'){
                    $q->where('fetured',true); 
                }elseif($r->status=='bestsale'){
                    $q->where('up_coming',true); 
                }else{
        ...
        ->select(['id','name','final_price','slug','view','type','brand_id','created_at','addedby_id','status','fetured','new_arrival'])
        ->paginate(25)->appends([
          'search'=>$r->search,
          'status'=>$r->status,
        ...
        ->selectRaw("count(case when new_arrival = true then 1 end) as newarrival")
        ->selectRaw("count(case when fetured = true then 1 end) as fetured")
        ->selectRaw("count(case when up_coming = true then 1 end) as bestsale")
        ->first();
    ...
    public function productsAction(Request $r,$action,$id=null){
        ...
        // return $product->productAttibutesVariationGroup()->sum('quantity');
        ...
        if($action=='duplicate'){
            ...
            foreach ($product->productVariationAttributeItems as $variationAttributeItem) {
                ...
                $newVariationAttributeItem->save();
        ...
        $product->seo_contents=$r->seo_contents;
        ...
        $product->subbrand_id=$r->sub_brand;
        $product->country_id=$r->country;
        ...
        $product->up_coming =$r->up_coming?1:0;
      ...
      $countries =Attribute::where('type',14)->where('status','<>','temp')->where('parent_id',null)->get();
      $attriMessage ='';
      $brand =$product->brand;

      return view(adminTheme().'products.productsEdit',compact('product','categories','brands','attributes','variationAttributes','tags','countries','brand','attriMessage'));
    ...
    public function productsUpdateAjax(Request $r,$column,$id){
      ...
      if($r->ajax() && $product){
        ...
        if($column=='attributesItemAddIds' || $column=='attributesVariationItemAddIds' || $column=='attributesVariationItemAdd' || $column=='attributesVariationItemUpdate' || $column=='attributesVariationImageUpdate' || $column=='attributesItemDeletesIds'){
            ...
            if($column=='attributesVariationItemAdd'){
                ...
                if($r->itemsIds && $product->productVariationAttributeItemsCheck($r->itemsIds)){
                  ...
                  $data->preorder_price = $r->variation_preorder_price?:0;
                  ...
                  $data->final_price =roundPriceUp($data->final_price);
            ...
            if($product->productAttibutesVariationGroup()->count() > 0){
                ...
                $product->quantity=$product->productVariationAttributeItems()->sum('quantity');
        ...
        if($column=='discount' || $column=='discount_type' || $column=='regular_price'){
              ...
              $product->final_price =roundPriceUp($product->final_price);
  ...
  public function productsTagsAction(Request $r,$action,$id=null){
    ...
    //Product Country Function
  public function productsCountries(Request $r){

      // Filter Action Start
      if($r->action){
        if($r->checkid){

        $datas=Attribute::where('type',14)->whereIn('id',$r->checkid)->get();
    ...
    //Filter Action End
      ...
      $countries =Attribute::latest()->where('type',14)->where('status','<>','temp')
      ->where(function($q) use($r){
    ...
    ->first();
      ...
      return view(adminTheme().'products.countries.countriesAll',compact('countries','totals','r'));
  }

  public function productsCountriesAction(Request $r,$action,$id=null){
      if($action=='create'){
        $country =Attribute::where('type',14)->where('status','temp')->where('addedby_id',Auth::id())->first();
        if(!$country){
          $country =new Attribute();
          $country->type =14;
          $country->status ='temp';
          $country->addedby_id =Auth::id();
        }
        $country->created_at=Carbon::now();
        $country->save();
        return redirect()->route('admin.productsCountriesAction',['edit',$country->id]);
      }

      $country =Attribute::where('type',14)->find($id);
      if(!$country){
        Session()->flash('error','This Country Are Not Found');
        return redirect()->route('admin.productsCountries');
      }


      if($action=='update'){
        $check = $r->validate([
            'name' => 'required|max:191|unique:attributes,name,'.$country->id,
        ]);

        $country->name=$r->name;
        $country->description=$r->description;
        $country->status =$r->status?'active':'inactive';
        $country->addedby_id =Auth::id();
        $country->save();
      ...
      return view(adminTheme().'products.countries.countryEdit',compact('country'));
  }

    //Product Country Function End
