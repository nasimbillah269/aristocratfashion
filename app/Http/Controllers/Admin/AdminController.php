...
//Brands Function End
        ...
        if($type=='products'){
            ...
            $products = Post::latest()->where('type',2)->where('status', '<>','temp')
                ->where(function($qq)  use ($r)  {
                    ...
                    if($r->stock)
                    {
                        if($r->stock=='out_of_stock' || $r->stock=='pre_order'){
                            $qq->where('quantity','<',1)->orWhere('quantity',null);
                        }else{
                            $qq->where('quantity','>',0);
                        }
                    }
            ...
            return view(adminTheme().'reports.customerReports',compact('customers','type'));
            
        }else{
            
            $customerOrders  = Order::latest()->where('order_type','customer_order')->where('order_status','<>','temp')
                            ->whereDate('created_at','>=',$from)->whereDate('created_at','<=',$to)->get(['id','grand_total','payment_status','order_status']);

            $report =array(
                    'sales'=>$customerOrders->sum('grand_total'),
                    'paid'=>$customerOrders->where('payment_status','paid')->sum('grand_total'),
                    'unpaid'=>$customerOrders->where('payment_status','unpaid')->sum('grand_total'),
                    'orders'=>$customerOrders->count(),
                    'pending'=>$customerOrders->where('order_status','pending')->count(),
                    'confirmed'=>$customerOrders->where('order_status','confirmed')->count(),
                    'delivered'=>$customerOrders->where('order_status','delivered')->count(),
                    'shipped'=>$customerOrders->where('order_status','shipped')->count(),
                    'cancelled'=>$customerOrders->where('order_status','cancelled')->count(),
                    'paidOrder'=>$customerOrders->where('payment_status','paid')->count(),
                );
             
            return view(adminTheme().'reports.summeryReports',compact('type','from','to','report'));
        }
    ...
    public function themeSetting(Request $r){
      ...
      return view(adminTheme().'theme-setting.themeSetting',compact('homeDatas','offerNotes','bannerNote','featuredNote','emis'));
    }
    
    public function themeSettingAction(Request $r,$action,$id=null){
        
        
        if (in_array($action, ['ami-add', 'ami-update', 'ami-delete'])) {
            ...
            // UPDATE (keyup)
            if ($action == 'ami-update') {
                $data = EmiCharge::find($r->id);
                if ($data) {
                    $data->update([
                        $r->field => $r->value
                    ]);
            ...
            $emis = EmiCharge::all();
            $view = view(adminTheme().'theme-setting.includes.emiList', compact('emis'))->render();
        ...
        if($action=="update"){
            ...
            $homedata->image_link2=$r->image_link2;
            $homedata->image_link3=$r->image_link3;
  ...
  public function userRoles(Request $r){
    ...
    return view(adminTheme().'users.roles.userRoles',compact('roles','r'));
  }
  ...
  public function setting($type){
    ...
    if($type=='general'){
      ...
      if(File::exists($general->footer_logo)){
            File::delete($general->footer_logo);
      }
      $general->footer_logo=null;
      $general->save();

      Session()->flash('success','Footer Logo Deleted Are Successfully Done!');
      return redirect()->back();
