...
<div class="content-header row">
    ...
    <div class="content-header-right col-md-6 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            ...
            <a class="btn btn-outline-success" href="{{route('admin.productsEdit',$product->id)}}">Edit</a>
            @endisset
            <a class="btn btn-outline-primary" href="{{route('admin.productsView',$product->id)}}">
    ...
    <section class="basic-elements">
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">     
                                <table class="table table-bordered">
                                    ...
                                    @foreach($product->purchases()->latest()->limit(100)->get() as $i=>$purchase)
                                    <tr>
                                        <td>{{$i+1}}</td>
                                        <td>
                                            @if($purchase->order)
                                            <a href="{{route('admin.purchaseProductsInvoice',$purchase->order->id)}}" target="_blank">
                                        ...
                                        <td>{{$purchase->order?$purchase->order->user?$purchase->order->user->name:'Not Found':'Not Found'}}</td>
                                        <td>{{$purchase->quantity}}</td>
                                        <td>{{$purchase->price}}</td>
                                        <td>{{$purchase->total_price}}</td>
                                        <td>{{$purchase->order?$purchase->order->created_at->format('d-m-Y'):''}}</td>
                                    </tr>
                                    @endforeach
                                </table>
                            </div>
                ...
                <div class="card">
                    <div class="card-header" style="border-bottom: 1px solid #e3ebf3;">
                        <h4 class="card-title">Transfers Report</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <div class="table-responsive">     
                                <table class="table table-bordered">
                                    ...
                                    @foreach($product->transfers()->latest()->limit(100)->get() as $i=>$transfer)
                                    <tr>
                                        <td>{{$i+1}}</td>
                                        <td>
                                            @if($transfer->order)
                                            <a href="{{route('admin.stockTransfersInvoice',$transfer->order->id)}}" target="_blank">
                                        ...
                                        <td>{{$transfer->branch?$transfer->branch->name:''}}</td>
                                        <td>{{$transfer->quantity}}</td>
                                        <td>{{$transfer->price}}</td>
                                        <td>{{$transfer->total_price}}</td>
                                        <td>{{$transfer->order?$transfer->order->created_at->format('d-m-Y'):''}}</td>
                                    </tr>
                                    @endforeach
                                </table>
                            </div>
