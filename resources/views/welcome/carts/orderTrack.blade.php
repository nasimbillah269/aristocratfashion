            ...
            <div class="order-serach">
                ...
                <div class="myrecentorder" style="margin-bottom: 10px;">
                    <div class="statustimeline">
                        @if($order->payment_method==null) @include(App\Models\General::first()->theme.'.customer.includes.orderpaydmentpending') @else @if($order->order_status=='pending')
                        @include(App\Models\General::first()->theme.'.customer.includes.orderpendingstatus') @elseif($order->order_status=='confirmed') @include(App\Models\General::first()->theme.'.customer.includes.orderconfirmstatus')
                        @elseif($order->order_status=='shipped') @include(App\Models\General::first()->theme.'.customer.includes.ordershipedstatus') @elseif($order->order_status=='delivered')
                        @include(App\Models\General::first()->theme.'.customer.includes.orderdeliveredstatus') @elseif($order->order_status=='Cancelled') @include(App\Models\General::first()->theme.'.customer.includes.ordercancelstatus')
                        @endif @endif
                    </div>
