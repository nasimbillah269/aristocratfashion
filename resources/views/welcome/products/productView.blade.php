...
<main class="mgmt-detail-section" id="mgmtDetailSection">
            ...
            <div class="col-12 col-lg-6">
                <div class="mgmt-detail-img-wrap largeImage">
                    ...
                    <div class="mgmt-thumbnails-wrap">
                        <div class="mgmt-thumbnail-item active" data-src="{{asset($product->image())}}">
                            <img src="{{asset($product->image())}}" alt="{{$product->name}}">
                        </div>
                        @if(isset($product->galleryFiles) && $product->galleryFiles->count() > 0)
                            @foreach($product->galleryFiles as $gallery)
                                <div class="mgmt-thumbnail-item" data-src="{{asset($gallery->image())}}">
        ...
        <p class="mgmt-detail-price fs-3 fw-bold text-danger">
            ...
            <div class="productPriceAppend">
                @if($selectVariation->stockStatus())
                    <span class="offerPrice" data-price="{{$selectVariation->offerPrice()}}">{{priceFullFormat($selectVariation->offerPrice())}}</span>
                    ...
                    @endif
                @else
                    <span class="offerPrice" data-price="{{$selectVariation->preorder_price}}">{{priceFullFormat($selectVariation->preorder_price)}}</span>
            ...
            @if($product->stockStatus())
                <span class="offerPrice" data-price="{{$product->offerPrice()}}">{{priceFullFormat($product->offerPrice())}}</span>
                ...
                @endif
            @else
                <span class="offerPrice" data-price="{{$product->purchase_price}}">{{priceFullFormat($product->purchase_price)}}</span>
    ...
    <form class="addToCartForm" action="{{route('addToCart',$product->id)}}" method="post">
        ...
        <div class="mgmt-detail-cart-row d-flex align-items-center gap-2 mb-3">
            ...
            <div class="mgmt-detail-qty-control quantityValue d-flex align-items-center border rounded">
                ...
                <input type="number" name="quantity" class="mgmt-detail-qty-input productQtyValue form-control text-center border-0 fw-bold" id="qty" value="1" min="1" data-max="{{$product->quantity}}" style="width: 50px;" readonly>
            ...
            <!-- Add to Cart -->
            <button type="button" class="mgmt-detail-btn-cart addToCart addToSinBtn btn btn-dark px-4 py-2 d-flex align-items-center gap-2" data-product-id="{{ $product->id }}" data-url="{{route('addToCart',$product->id)}}">
            ...
            <!-- Buy Now -->
            <button type="submit" name="orderNow" value="order" class="mgmt-detail-btn-buy buyNow buyNowSinBtn btn text-white px-4 py-2 d-flex align-items-center gap-2 m-0" data-product-id="{{ $product->id }}" style="background-color: #e91e63;">
  ...
  <div class="feature-item">
    ...
    <div class="custom-cod-btn-wrap my-3">
        ...
        @if($banner1)
            <a href="{{$banner1->image_link?:'javascript:void(0)'}}" style="display: block;box-shadow: 0px 2px 7px rgb(0 0 0 / 15%);border-radius: 5px;margin-bottom: 10px;" >
                <img src="{{ asset($banner1->image()) }}" alt="{{$banner1->name}}" style="border-radius: 5px;" />
            </a>
        @endif
        
        @if($banner2)
            <a href="{{$banner2->image_link?:'javascript:void(0)'}}" style="display: block;box-shadow: 0px 2px 7px rgb(0 0 0 / 15%);border-radius: 5px;margin-bottom: 10px;" >
                <img src="{{ asset($banner2->image()) }}" alt="{{$banner2->name}}" style="border-radius: 5px;" />
            </a>
        @endif
    ...
    <div class="mgmt-detail-meta-wrap text-muted small mt-4">
        @if($product->sku_code)
            <p class="mgmt-detail-meta-line mb-1"><strong>SKU:</strong> <span>{{$product->sku_code}}</span></p>
        @endif

        @if($product->country)
            <p class="mgmt-detail-meta-line mb-1"><strong>Country:</strong> <span>{{$product->country->name}}</span></p>
        @endif

        <p class="mgmt-detail-meta-line mb-1"><strong>Categories:</strong>
            @foreach($product->productCategories as $ctg)
                <a href="{{ route('productCategory', $ctg->slug ?: 'no-title') }}" class="mgmt-detail-meta-link text-decoration-none text-primary">{{ $ctg->name }}</a>{{ !$loop->last ? ',' : '' }}
            @endforeach
        </p>
        
        <p class="mgmt-detail-meta-line mb-1"><strong>Available:</strong> <span>{{$product->quantity}} Qty</span></p>
    </div>
    ...
    <div class="mgmt-detail-share-wrap d-flex align-items-center gap-2 mt-3">
        ...
        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('productView', $product->slug ?: 'no-title')) }}" class="btn btn-primary rounded-circle p-0 d-inline-flex align-items-center justify-content-center" style="width: 35px; height: 35px;" aria-label="Share on Facebook"><i class="fab fa-facebook-f"></i></a>
        <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('productView', $product->slug ?: 'no-title')) }}&text={{ urlencode($product->name) }}" class="btn btn-info text-white rounded-circle p-0 d-inline-flex align-items-center justify-content-center" style="width: 35px; height: 35px;" aria-label="Share on X"><i class="fab fa-twitter"></i></a>
        ...
        <a href="https://api.whatsapp.com/send?text={{ urlencode($product->name . ' - ' . route('productView', $product->slug ?: 'no-title')) }}" class="btn btn-success rounded-circle p-0 d-inline-flex align-items-center justify-content-center" style="width: 35px; height: 35px;" aria-label="Share on WhatsApp"><i class="fab fa-whatsapp"></i></a>
...
<section class="mgmt-product-tabs-section" id="mgmtProductTabs">
    <div class="container">
        ...
        <div class="mgmt-product-tab-content active" id="mgmtTabDescription">
            <div class="mgmt-description-content paragraphs">{!! $product->description !!}</div>
        </div>
        <!-- ========================================= SPECIFICATION TAB ========================================== -->
        ...
        <div class="mgmt-product-tab-content" id="mgmtTabReviews">
            <div class="mgmt-review-wrapper">
                <div class="ReviewList">
                    @include(welcomeTheme() . '.products.includes.realatedReview')
                </div>
                <!-- REVIEW FORM -->
                <div class="mgmt-review-form-wrapper">
                    ...
                    <form action="{{ route('productReview', $product->slug ?: Str::slug($product->name)) }}" method="POST" id="productReviewForm" class="mgmt-review-form">
                        @csrf
    ...
    <div class="container">
        ...
        <div class="row g-3 g-md-4">
            @foreach($relatedProducts as $relatedProd)
                <div class="col-md-3">
                    @include(welcomeTheme().'products.includes.productCard', ['product' => $relatedProd])
                </div>
            @endforeach
        </div>
    </div>
